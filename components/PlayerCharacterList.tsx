...
const PlayerCharacterList: React.FC<PlayerCharacterListProps> = ({
  ...
  return (
    <div className="max-w-3xl mx-auto my-8 p-6">
      <h2 className="text-3xl font-bold text-sky-800 dark:text-sky-300 mb-8 text-center">Meus Personagens</h2>
      {characters.length === 0 ? (
        <div className="text-center p-6 bg-white dark:bg-slate-800 shadow-md rounded-lg">
          <p className="text-black dark:text-slate-200 text-lg mb-4">Você ainda não criou nenhum personagem.</p>
        </div>
      ) : (
          ...
          {characters.map((char) => (
            ...
            >
              <div className="flex items-center mb-3 sm:mb-0">
                ...
                />
                <div>
                  <h3 className="text-lg font-semibold text-sky-700 dark:text-sky-400">{char.name}</h3>
