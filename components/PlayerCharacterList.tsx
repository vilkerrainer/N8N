...
const PlayerCharacterList: React.FC<PlayerCharacterListProps> = ({
  ...
  return (
    <div className="max-w-3xl mx-auto my-8 p-6">
      <h2 className="text-3xl font-bold text-sky-800 mb-8 text-center">Meus Personagens</h2>
      {characters.length === 0 ? (
        <div className="text-center p-6 bg-white shadow-md rounded-lg">
          <p className="text-black text-lg mb-4">Você ainda não criou nenhum personagem.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {characters.map((char) => (
            <div
              key={char.id}
              className="bg-white p-4 rounded-lg shadow-md flex flex-col sm:flex-row justify-between items-center"
            >
              <div className="flex items-center mb-3 sm:mb-0">
                ...
                />
                <div>
                  <h3 className="text-lg font-semibold text-sky-700">{char.name}</h3>
              ...
              <div className="flex space-x-2 flex-shrink-0">
                ...
                <Button 
                  ...
                  variant="secondary" 
                  className="text-xs px-3 py-1 bg-red-500 hover:bg-red-600 text-white focus:ring-red-400"
