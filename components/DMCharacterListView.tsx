...
const DMCharacterListView: React.FC<DMCharacterListViewProps> = ({ characters, onDMUpdateCharacter, onDeleteCharacter }) => {
  ...
  if (characters.length === 0) {
    return (
      <div className="max-w-4xl mx-auto my-8 p-6 bg-white dark:bg-slate-800 shadow-xl rounded-lg text-center">
        <h2 className="text-3xl font-bold text-sky-700 dark:text-sky-400 mb-6">Visão do Mestre</h2>
        <p className="text-slate-800 dark:text-slate-100 text-lg">Nenhum personagem foi criado ainda (ou o personagem de teste não carregou).</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto my-8 p-6">
      <h2 className="text-3xl font-bold text-sky-700 dark:text-sky-400 mb-6 text-center">Visão do Mestre - Personagens Criados</h2>
          ...
          return (
            <div key={char.id} className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-lg hover:shadow-2xl transition-shadow duration-300 flex flex-col">
              ...
              <div className="space-y-2 text-sm mb-4 flex-grow">
                <div className="flex justify-between p-2 bg-slate-100 dark:bg-slate-700/60 rounded">
                  <span className="font-semibold text-slate-700 dark:text-slate-300">HP:</span>
                  <span className="text-slate-800 dark:text-slate-100">{char.hp} / {char.hpt}</span>
                </div>
                <div className="flex justify-between p-2 bg-slate-100 dark:bg-slate-700/60 rounded">
                  <span className="font-semibold text-slate-700 dark:text-slate-300">CA:</span>
                  <span className="text-slate-800 dark:text-slate-100">{char.ac}</span>
                </div>
                <div className="flex justify-between p-2 bg-slate-100 dark:bg-slate-700/60 rounded">
                ...
                {showAttributes && (
                  <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-600">
                    <h4 className="text-sm font-semibold text-slate-600 dark:text-slate-400 mb-1">Atributos:</h4>
                ...
                {showSkills && (
                  <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-600">
                    <h4 className="text-sm font-semibold text-slate-600 dark:text-slate-400 mb-1">Perícias: <span className="text-xs font-normal">(Bônus Prof: {formatModifier(proficiencyBonus)})</span></h4>
                    <div className="text-xs space-y-1 max-h-48 overflow-y-auto pr-1">
                      {ALL_SKILLS.map((skill: SkillDefinition) => {
                        ...
                        return (
                          <div key={skill.key} className={`flex justify-between items-center p-1 rounded ${isProficient ? 'bg-sky-100 dark:bg-sky-700/50 shadow-sm' : 'bg-slate-100 dark:bg-slate-700/50'}`}>
                            <span className={` ${isProficient ? 'text-sky-700 dark:text-sky-400 font-semibold' : 'text-slate-700 dark:text-slate-300'}`}>
                              {isProficient && <span title="Proficiente">● </span>}{skill.label} 
                              <span className="text-slate-500 dark:text-slate-400 text-xs ml-1">({ATTRIBUTE_LABELS[skill.attribute].substring(0,3)})</span>
                            </span>
                            <span className={`px-2 py-0.5 rounded text-xs ${isProficient ? 'text-sky-800 dark:text-sky-100 bg-sky-200 dark:bg-sky-600 font-bold' : 'text-slate-800 dark:text-slate-200 bg-slate-200 dark:bg-slate-600'}`}>{formatModifier(skillModifier)}</span>
                          </div>
                        );
                      })}
                    </div>
              ...
              {onDMUpdateCharacter && (
                <div className="mt-4 border-t border-slate-200 dark:border-slate-600 pt-4 space-y-3">
                  <div>
                    <Input
                      label="Cobrar Moedas"
                      id={`charge-${char.id}`}
